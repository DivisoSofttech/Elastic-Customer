...
public class CustomerResourceIntTest {
     ...
     */
	/*
	 * @Autowired private CustomerSearchRepository mockCustomerSearchRepository;
	 */
    @Autowired
	 ...
	 * @Test
	 * 
	 * @Transactional public void createCustomer() throws Exception { int
	 * databaseSizeBeforeCreate = customerRepository.findAll().size();
	 * 
	 * // Create the Customer CustomerDTO customerDTO =
	 ...
	 * 
	 * // Validate the Customer in the database List<Customer> customerList =
	 * customerRepository.findAll();
	 * assertThat(customerList).hasSize(databaseSizeBeforeCreate + 1); Customer
	 * testCustomer = customerList.get(customerList.size() - 1);
	 ...
	 * assertThat(testCustomer.getPhotoContentType()).isEqualTo(
	 ...
	 * @Test
	 * 
	 * @Transactional public void createCustomerWithExistingId() throws Exception {
	 * int databaseSizeBeforeCreate = customerRepository.findAll().size();
	 * 
	 * // Create the Customer with an existing ID customer.setId(1L); CustomerDTO
	 * customerDTO = customerMapper.toDto(customer);
	 * 
	 * // An entity with an existing ID cannot be created, so this API call must
	 * fail restCustomerMockMvc.perform(post("/api/customers")
	 * .contentType(TestUtil.APPLICATION_JSON_UTF8)
	 * .content(TestUtil.convertObjectToJsonBytes(customerDTO)))
	 ...
	 * @Test
	 * 
	 * @Transactional public void getAllCustomers() throws Exception { // Initialize
	 * the database customerRepository.saveAndFlush(customer);
	 * 
	 ...
	 * .andExpect(jsonPath("$.[*].curDebt").value(hasItem(DEFAULT_CUR_DEBT.
	 * doubleValue())))
	 * .andExpect(jsonPath("$.[*].debtDate").value(hasItem(DEFAULT_DEBT_DATE.
	 * toString())))
	 * .andExpect(jsonPath("$.[*].maxDebt").value(hasItem(DEFAULT_MAX_DEBT.
	 * doubleValue())))
	 * .andExpect(jsonPath("$.[*].discount").value(hasItem(DEFAULT_DISCOUNT.
	 * doubleValue())))
	 * .andExpect(jsonPath("$.[*].visible").value(hasItem(DEFAULT_VISIBLE.
	 * booleanValue())))
	 * .andExpect(jsonPath("$.[*].photoContentType").value(hasItem(
	 * DEFAULT_PHOTO_CONTENT_TYPE)))
	 * .andExpect(jsonPath("$.[*].photo").value(hasItem(Base64Utils.encodeToString(
	 * DEFAULT_PHOTO)))); }
	 * 
	 * @Test
	 * 
	 * @Transactional public void getCustomer() throws Exception { // Initialize the
	 * database customerRepository.saveAndFlush(customer);
	 * 
	 * // Get the customer restCustomerMockMvc.perform(get("/api/customers/{id}",
	 * customer.getId())) .andExpect(status().isOk())
	 * .andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8_VALUE))
	 ...
	 * @Test
	 * 
	 * @Transactional public void getNonExistingCustomer() throws Exception { // Get
	 * the customer restCustomerMockMvc.perform(get("/api/customers/{id}",
	 * Long.MAX_VALUE)) .andExpect(status().isNotFound()); }
	 * 
	 * @Test
	 * 
	 * @Transactional public void updateCustomer() throws Exception { // Initialize
	 * the database customerRepository.saveAndFlush(customer);
	 * 
	 * int databaseSizeBeforeUpdate = customerRepository.findAll().size();
	 * 
	 * // Update the customer Customer updatedCustomer =
	 * customerRepository.findById(customer.getId()).get(); // Disconnect from
	 * session so that the updates on updatedCustomer are not directly saved in db
	 * em.detach(updatedCustomer); updatedCustomer .reference(UPDATED_REFERENCE)
	 * .name(UPDATED_NAME) .searchKey(UPDATED_SEARCH_KEY) .card(UPDATED_CARD)
	 * .curDebt(UPDATED_CUR_DEBT) .debtDate(UPDATED_DEBT_DATE)
	 * .maxDebt(UPDATED_MAX_DEBT) .discount(UPDATED_DISCOUNT)
	 * .visible(UPDATED_VISIBLE) .photo(UPDATED_PHOTO)
	 * .photoContentType(UPDATED_PHOTO_CONTENT_TYPE); CustomerDTO customerDTO =
	 ...
	 * // Validate the Customer in the database List<Customer> customerList =
	 * customerRepository.findAll();
	 * assertThat(customerList).hasSize(databaseSizeBeforeUpdate); Customer
	 * testCustomer = customerList.get(customerList.size() - 1);
	 ...
	 * @Test
	 * 
	 * @Transactional public void updateNonExistingCustomer() throws Exception { int
	 * databaseSizeBeforeUpdate = customerRepository.findAll().size();
	 * 
	 * // Create the Customer CustomerDTO customerDTO =
	 * customerMapper.toDto(customer);
	 * 
	 ...
	 * @Test
	 * 
	 * @Transactional public void deleteCustomer() throws Exception { // Initialize
	 * the database customerRepository.saveAndFlush(customer);
	 * 
	 ...
	 * 
	 * // Validate the database is empty List<Customer> customerList =
	 * customerRepository.findAll();
	 ...
	 * @Test
	 * 
	 * @Transactional public void searchCustomer() throws Exception { // Initialize
	 * the database customerRepository.saveAndFlush(customer);
	 * when(mockCustomerSearchRepository.search(queryStringQuery("id:" +
	 * customer.getId()), PageRequest.of(0, 20))) .thenReturn(new
	 * PageImpl<>(Collections.singletonList(customer), PageRequest.of(0, 1), 1)); //
	 * Search the customer
	 * restCustomerMockMvc.perform(get("/api/_search/customers?query=id:" +
	 * customer.getId())) .andExpect(status().isOk())
	 * .andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8_VALUE))
	 * .andExpect(jsonPath("$.[*].id").value(hasItem(customer.getId().intValue())))
	 * .andExpect(jsonPath("$.[*].reference").value(hasItem(DEFAULT_REFERENCE)))
	 ...
	 * TestUtil.equalsVerifier(Customer.class); Customer customer1 = new Customer();
	 * customer1.setId(1L); Customer customer2 = new Customer();
	 * customer2.setId(customer1.getId());
	 * assertThat(customer1).isEqualTo(customer2); customer2.setId(2L);
	 * assertThat(customer1).isNotEqualTo(customer2); customer1.setId(null);
	 * assertThat(customer1).isNotEqualTo(customer2); }
	 * 
	 * @Test
	 * 
	 * @Transactional public void dtoEqualsVerifier() throws Exception {
	 * TestUtil.equalsVerifier(CustomerDTO.class); CustomerDTO customerDTO1 = new
	 * CustomerDTO(); customerDTO1.setId(1L); CustomerDTO customerDTO2 = new
	 * CustomerDTO(); assertThat(customerDTO1).isNotEqualTo(customerDTO2);
	 * customerDTO2.setId(customerDTO1.getId());
	 * assertThat(customerDTO1).isEqualTo(customerDTO2); customerDTO2.setId(2L);
