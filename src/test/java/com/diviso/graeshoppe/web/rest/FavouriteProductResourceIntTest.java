...
public class FavouriteProductResourceIntTest {
     ...
     */
	/*
	 * @Autowired private FavouriteProductSearchRepository
	 ...
	 * @Test
	 * 
	 * @Transactional public void createFavouriteProduct() throws Exception { int
	 * databaseSizeBeforeCreate = favouriteProductRepository.findAll().size();
	 * 
	 * // Create the FavouriteProduct FavouriteProductDTO favouriteProductDTO =
	 * favouriteProductMapper.toDto(favouriteProduct);
	 ...
	 * 
	 * // Validate the FavouriteProduct in the database List<FavouriteProduct>
	 * favouriteProductList = favouriteProductRepository.findAll();
	 * assertThat(favouriteProductList).hasSize(databaseSizeBeforeCreate + 1);
	 * FavouriteProduct testFavouriteProduct =
	 * favouriteProductList.get(favouriteProductList.size() - 1);
	 * assertThat(testFavouriteProduct.getProductId()).isEqualTo(DEFAULT_PRODUCT_ID)
	 * ;
	 * 
	 * // Validate the FavouriteProduct in Elasticsearch
	 * verify(mockFavouriteProductSearchRepository,
	 * times(1)).save(testFavouriteProduct); }
	 * 
	 * @Test
	 * 
	 * @Transactional public void createFavouriteProductWithExistingId() throws
	 * Exception { int databaseSizeBeforeCreate =
	 * favouriteProductRepository.findAll().size();
	 * 
	 * // Create the FavouriteProduct with an existing ID
	 * favouriteProduct.setId(1L); FavouriteProductDTO favouriteProductDTO =
	 * favouriteProductMapper.toDto(favouriteProduct);
	 * 
	 * // An entity with an existing ID cannot be created, so this API call must
	 * fail restFavouriteProductMockMvc.perform(post("/api/favourite-products")
	 * .contentType(TestUtil.APPLICATION_JSON_UTF8)
	 * .content(TestUtil.convertObjectToJsonBytes(favouriteProductDTO)))
	 ...
	 * // Validate the FavouriteProduct in Elasticsearch
	 * verify(mockFavouriteProductSearchRepository,
	 * times(0)).save(favouriteProduct); }
	 * 
	 * @Test
	 * 
	 * @Transactional public void getAllFavouriteProducts() throws Exception { //
	 * Initialize the database
	 * favouriteProductRepository.saveAndFlush(favouriteProduct);
	 * 
	 * // Get all the favouriteProductList restFavouriteProductMockMvc.perform(get(
	 * "/api/favourite-products?sort=id,desc")) .andExpect(status().isOk())
	 * .andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8_VALUE))
	 * .andExpect(jsonPath("$.[*].id").value(hasItem(favouriteProduct.getId().
	 * intValue())))
	 ...
	 * @Test
	 * 
	 * @Transactional public void getFavouriteProduct() throws Exception { //
	 * Initialize the database
	 * favouriteProductRepository.saveAndFlush(favouriteProduct);
	 * 
	 * // Get the favouriteProduct
	 * restFavouriteProductMockMvc.perform(get("/api/favourite-products/{id}",
	 * favouriteProduct.getId())) .andExpect(status().isOk())
	 * .andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8_VALUE))
	 ...
	 * @Test
	 * 
	 * @Transactional public void getNonExistingFavouriteProduct() throws Exception
	 * { // Get the favouriteProduct
	 * restFavouriteProductMockMvc.perform(get("/api/favourite-products/{id}",
	 ...
	 * @Test
	 * 
	 * @Transactional public void updateFavouriteProduct() throws Exception { //
	 * Initialize the database
	 * favouriteProductRepository.saveAndFlush(favouriteProduct);
	 * 
	 * int databaseSizeBeforeUpdate = favouriteProductRepository.findAll().size();
	 * 
	 * // Update the favouriteProduct FavouriteProduct updatedFavouriteProduct =
	 * favouriteProductRepository.findById(favouriteProduct.getId()).get(); //
	 * Disconnect from session so that the updates on updatedFavouriteProduct are
	 * not directly saved in db em.detach(updatedFavouriteProduct);
	 * updatedFavouriteProduct .productId(UPDATED_PRODUCT_ID); FavouriteProductDTO
	 * favouriteProductDTO = favouriteProductMapper.toDto(updatedFavouriteProduct);
	 * 
	 ...
	 * @Test
	 * 
	 * @Transactional public void updateNonExistingFavouriteProduct() throws
	 ...
	 * @Test
	 * 
	 * @Transactional public void deleteFavouriteProduct() throws Exception { //
	 * Initialize the database
	 * favouriteProductRepository.saveAndFlush(favouriteProduct);
	 * 
	 ...
	 * restFavouriteProductMockMvc.perform(delete("/api/favourite-products/{id}",
	 * favouriteProduct.getId()) .accept(TestUtil.APPLICATION_JSON_UTF8))
	 * .andExpect(status().isOk());
	 * 
	 * // Validate the database is empty List<FavouriteProduct> favouriteProductList
	 * = favouriteProductRepository.findAll();
	 * assertThat(favouriteProductList).hasSize(databaseSizeBeforeDelete - 1);
	 * 
	 * // Validate the FavouriteProduct in Elasticsearch
	 * verify(mockFavouriteProductSearchRepository,
	 * times(1)).deleteById(favouriteProduct.getId()); }
	 * 
	 * @Test
	 * 
	 * @Transactional public void searchFavouriteProduct() throws Exception { //
	 * Initialize the database
	 * favouriteProductRepository.saveAndFlush(favouriteProduct);
	 * when(mockFavouriteProductSearchRepository.search(queryStringQuery("id:" +
	 * favouriteProduct.getId()), PageRequest.of(0, 20))) .thenReturn(new
	 * PageImpl<>(Collections.singletonList(favouriteProduct), PageRequest.of(0, 1),
	 * 1)); // Search the favouriteProduct restFavouriteProductMockMvc.perform(get(
	 * "/api/_search/favourite-products?query=id:" + favouriteProduct.getId()))
	 * .andExpect(status().isOk())
	 * .andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8_VALUE))
	 * .andExpect(jsonPath("$.[*].id").value(hasItem(favouriteProduct.getId().
	 * intValue())))
	 ...
	 * TestUtil.equalsVerifier(FavouriteProduct.class); FavouriteProduct
	 * favouriteProduct1 = new FavouriteProduct(); favouriteProduct1.setId(1L);
	 ...
	 * TestUtil.equalsVerifier(FavouriteProductDTO.class); FavouriteProductDTO
	 * favouriteProductDTO1 = new FavouriteProductDTO();
	 * favouriteProductDTO1.setId(1L); FavouriteProductDTO favouriteProductDTO2 =
	 * new FavouriteProductDTO();
