...
public class FavouriteStoreResourceIntTest {
     ...
     */
	/*
	 * @Autowired private FavouriteStoreSearchRepository
	 * mockFavouriteStoreSearchRepository;
	 */
    @Autowired
	 ...
	 * @Test
	 * 
	 * @Transactional public void createFavouriteStore() throws Exception { int
	 * databaseSizeBeforeCreate = favouriteStoreRepository.findAll().size();
	 * 
	 * // Create the FavouriteStore FavouriteStoreDTO favouriteStoreDTO =
	 * favouriteStoreMapper.toDto(favouriteStore);
	 ...
	 * 
	 * // Validate the FavouriteStore in the database List<FavouriteStore>
	 * favouriteStoreList = favouriteStoreRepository.findAll();
	 * assertThat(favouriteStoreList).hasSize(databaseSizeBeforeCreate + 1);
	 * FavouriteStore testFavouriteStore =
	 * favouriteStoreList.get(favouriteStoreList.size() - 1);
	 * assertThat(testFavouriteStore.getStoreId()).isEqualTo(DEFAULT_STORE_ID);
	 * 
	 * // Validate the FavouriteStore in Elasticsearch
	 * verify(mockFavouriteStoreSearchRepository,
	 * times(1)).save(testFavouriteStore); }
	 * 
	 * @Test
	 * 
	 * @Transactional public void createFavouriteStoreWithExistingId() throws
	 * Exception { int databaseSizeBeforeCreate =
	 * favouriteStoreRepository.findAll().size();
	 * 
	 * // Create the FavouriteStore with an existing ID favouriteStore.setId(1L);
	 * FavouriteStoreDTO favouriteStoreDTO =
	 * favouriteStoreMapper.toDto(favouriteStore);
	 * 
	 * // An entity with an existing ID cannot be created, so this API call must
	 * fail restFavouriteStoreMockMvc.perform(post("/api/favourite-stores")
	 * .contentType(TestUtil.APPLICATION_JSON_UTF8)
	 * .content(TestUtil.convertObjectToJsonBytes(favouriteStoreDTO)))
	 ...
	 * @Test
	 * 
	 * @Transactional public void getAllFavouriteStores() throws Exception { //
	 ...
	 * @Test
	 * 
	 * @Transactional public void getFavouriteStore() throws Exception { //
	 * Initialize the database
	 * favouriteStoreRepository.saveAndFlush(favouriteStore);
	 * 
	 * // Get the favouriteStore
	 * restFavouriteStoreMockMvc.perform(get("/api/favourite-stores/{id}",
	 * favouriteStore.getId())) .andExpect(status().isOk())
	 * .andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8_VALUE))
	 ...
	 * @Test
	 * 
	 * @Transactional public void getNonExistingFavouriteStore() throws Exception {
	 * // Get the favouriteStore
	 * restFavouriteStoreMockMvc.perform(get("/api/favourite-stores/{id}",
	 ...
	 * @Test
	 * 
	 * @Transactional public void updateFavouriteStore() throws Exception { //
	 * Initialize the database
	 * favouriteStoreRepository.saveAndFlush(favouriteStore);
	 * 
	 * int databaseSizeBeforeUpdate = favouriteStoreRepository.findAll().size();
	 * 
	 * // Update the favouriteStore FavouriteStore updatedFavouriteStore =
	 * favouriteStoreRepository.findById(favouriteStore.getId()).get(); //
	 * Disconnect from session so that the updates on updatedFavouriteStore are not
	 * directly saved in db em.detach(updatedFavouriteStore); updatedFavouriteStore
	 * .storeId(UPDATED_STORE_ID); FavouriteStoreDTO favouriteStoreDTO =
	 * favouriteStoreMapper.toDto(updatedFavouriteStore);
	 * 
	 ...
	 * @Test
	 * 
	 * @Transactional public void updateNonExistingFavouriteStore() throws Exception
	 * { int databaseSizeBeforeUpdate = favouriteStoreRepository.findAll().size();
	 * 
	 * // Create the FavouriteStore FavouriteStoreDTO favouriteStoreDTO =
	 * favouriteStoreMapper.toDto(favouriteStore);
	 ...
	 * @Test
	 * 
	 * @Transactional public void deleteFavouriteStore() throws Exception { //
	 * Initialize the database
	 * favouriteStoreRepository.saveAndFlush(favouriteStore);
	 * 
	 ...
	 * restFavouriteStoreMockMvc.perform(delete("/api/favourite-stores/{id}",
	 * favouriteStore.getId()) .accept(TestUtil.APPLICATION_JSON_UTF8))
	 * .andExpect(status().isOk());
	 * 
	 * // Validate the database is empty List<FavouriteStore> favouriteStoreList =
	 * favouriteStoreRepository.findAll();
	 * assertThat(favouriteStoreList).hasSize(databaseSizeBeforeDelete - 1);
	 * 
	 * // Validate the FavouriteStore in Elasticsearch
	 * verify(mockFavouriteStoreSearchRepository,
	 * times(1)).deleteById(favouriteStore.getId()); }
	 * 
	 * @Test
	 * 
	 * @Transactional public void searchFavouriteStore() throws Exception { //
	 * Initialize the database
	 * favouriteStoreRepository.saveAndFlush(favouriteStore);
	 * when(mockFavouriteStoreSearchRepository.search(queryStringQuery("id:" +
	 * favouriteStore.getId()), PageRequest.of(0, 20))) .thenReturn(new
	 * PageImpl<>(Collections.singletonList(favouriteStore), PageRequest.of(0, 1),
	 * 1)); // Search the favouriteStore restFavouriteStoreMockMvc.perform(get(
	 * "/api/_search/favourite-stores?query=id:" + favouriteStore.getId()))
	 * .andExpect(status().isOk())
	 * .andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8_VALUE))
	 * .andExpect(jsonPath("$.[*].id").value(hasItem(favouriteStore.getId().intValue
	 * ())))
	 ...
	 * TestUtil.equalsVerifier(FavouriteStore.class); FavouriteStore favouriteStore1
	 * = new FavouriteStore(); favouriteStore1.setId(1L); FavouriteStore
	 * favouriteStore2 = new FavouriteStore();
	 ...
	 * TestUtil.equalsVerifier(FavouriteStoreDTO.class); FavouriteStoreDTO
	 * favouriteStoreDTO1 = new FavouriteStoreDTO(); favouriteStoreDTO1.setId(1L);
