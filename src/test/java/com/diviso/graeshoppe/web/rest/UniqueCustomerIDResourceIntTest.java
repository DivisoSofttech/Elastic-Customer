...
public class UniqueCustomerIDResourceIntTest {
     ...
     */
	/*
	 * @Autowired private UniqueCustomerIDSearchRepository
	 ...
	 * @Test
	 * 
	 * @Transactional public void createUniqueCustomerID() throws Exception { int
	 * databaseSizeBeforeCreate = uniqueCustomerIDRepository.findAll().size();
	 * 
	 * // Create the UniqueCustomerID UniqueCustomerIDDTO uniqueCustomerIDDTO =
	 * uniqueCustomerIDMapper.toDto(uniqueCustomerID);
	 ...
	 * 
	 * // Validate the UniqueCustomerID in the database List<UniqueCustomerID>
	 * uniqueCustomerIDList = uniqueCustomerIDRepository.findAll();
	 * assertThat(uniqueCustomerIDList).hasSize(databaseSizeBeforeCreate + 1);
	 * UniqueCustomerID testUniqueCustomerID =
	 * uniqueCustomerIDList.get(uniqueCustomerIDList.size() - 1);
	 * 
	 * // Validate the UniqueCustomerID in Elasticsearch
	 * verify(mockUniqueCustomerIDSearchRepository,
	 * times(1)).save(testUniqueCustomerID); }
	 * 
	 * @Test
	 * 
	 * @Transactional public void createUniqueCustomerIDWithExistingId() throws
	 * Exception { int databaseSizeBeforeCreate =
	 * uniqueCustomerIDRepository.findAll().size();
	 * 
	 * // Create the UniqueCustomerID with an existing ID
	 * uniqueCustomerID.setId(1L); UniqueCustomerIDDTO uniqueCustomerIDDTO =
	 * uniqueCustomerIDMapper.toDto(uniqueCustomerID);
	 * 
	 * // An entity with an existing ID cannot be created, so this API call must
	 * fail restUniqueCustomerIDMockMvc.perform(post("/api/unique-customer-ids")
	 * .contentType(TestUtil.APPLICATION_JSON_UTF8)
	 * .content(TestUtil.convertObjectToJsonBytes(uniqueCustomerIDDTO)))
	 ...
	 * // Validate the UniqueCustomerID in Elasticsearch
	 * verify(mockUniqueCustomerIDSearchRepository,
	 * times(0)).save(uniqueCustomerID); }
	 * 
	 * @Test
	 * 
	 * @Transactional public void getAllUniqueCustomerIDS() throws Exception { //
	 * Initialize the database
	 * uniqueCustomerIDRepository.saveAndFlush(uniqueCustomerID);
	 * 
	 * // Get all the uniqueCustomerIDList restUniqueCustomerIDMockMvc.perform(get(
	 * "/api/unique-customer-ids?sort=id,desc")) .andExpect(status().isOk())
	 * .andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8_VALUE))
	 * .andExpect(jsonPath("$.[*].id").value(hasItem(uniqueCustomerID.getId().
	 * intValue()))); }
	 * 
	 * @Test
	 * 
	 * @Transactional public void getUniqueCustomerID() throws Exception { //
	 * Initialize the database
	 * uniqueCustomerIDRepository.saveAndFlush(uniqueCustomerID);
	 * 
	 * // Get the uniqueCustomerID
	 * restUniqueCustomerIDMockMvc.perform(get("/api/unique-customer-ids/{id}",
	 * uniqueCustomerID.getId())) .andExpect(status().isOk())
	 * .andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8_VALUE))
	 * .andExpect(jsonPath("$.id").value(uniqueCustomerID.getId().intValue())); }
	 * 
	 * @Test
	 * 
	 * @Transactional public void getNonExistingUniqueCustomerID() throws Exception
	 * { // Get the uniqueCustomerID
	 * restUniqueCustomerIDMockMvc.perform(get("/api/unique-customer-ids/{id}",
	 ...
	 * @Test
	 * 
	 * @Transactional public void updateUniqueCustomerID() throws Exception { //
	 * Initialize the database
	 * uniqueCustomerIDRepository.saveAndFlush(uniqueCustomerID);
	 * 
	 * int databaseSizeBeforeUpdate = uniqueCustomerIDRepository.findAll().size();
	 * 
	 * // Update the uniqueCustomerID UniqueCustomerID updatedUniqueCustomerID =
	 * uniqueCustomerIDRepository.findById(uniqueCustomerID.getId()).get(); //
	 * Disconnect from session so that the updates on updatedUniqueCustomerID are
	 * not directly saved in db em.detach(updatedUniqueCustomerID);
	 * UniqueCustomerIDDTO uniqueCustomerIDDTO =
	 * uniqueCustomerIDMapper.toDto(updatedUniqueCustomerID);
	 * 
	 ...
	 * @Test
	 * 
	 * @Transactional public void updateNonExistingUniqueCustomerID() throws
	 ...
	 * @Test
	 * 
	 * @Transactional public void deleteUniqueCustomerID() throws Exception { //
	 * Initialize the database
	 * uniqueCustomerIDRepository.saveAndFlush(uniqueCustomerID);
	 * 
	 ...
	 * restUniqueCustomerIDMockMvc.perform(delete("/api/unique-customer-ids/{id}",
	 * uniqueCustomerID.getId()) .accept(TestUtil.APPLICATION_JSON_UTF8))
	 * .andExpect(status().isOk());
	 * 
	 * // Validate the database is empty List<UniqueCustomerID> uniqueCustomerIDList
	 * = uniqueCustomerIDRepository.findAll();
	 * assertThat(uniqueCustomerIDList).hasSize(databaseSizeBeforeDelete - 1);
	 * 
	 * // Validate the UniqueCustomerID in Elasticsearch
	 * verify(mockUniqueCustomerIDSearchRepository,
	 * times(1)).deleteById(uniqueCustomerID.getId()); }
	 * 
	 * @Test
	 * 
	 * @Transactional public void searchUniqueCustomerID() throws Exception { //
	 * Initialize the database
	 * uniqueCustomerIDRepository.saveAndFlush(uniqueCustomerID);
	 * when(mockUniqueCustomerIDSearchRepository.search(queryStringQuery("id:" +
	 * uniqueCustomerID.getId()), PageRequest.of(0, 20))) .thenReturn(new
	 * PageImpl<>(Collections.singletonList(uniqueCustomerID), PageRequest.of(0, 1),
	 * 1)); // Search the uniqueCustomerID restUniqueCustomerIDMockMvc.perform(get(
	 * "/api/_search/unique-customer-ids?query=id:" + uniqueCustomerID.getId()))
	 * .andExpect(status().isOk())
	 * .andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8_VALUE))
	 * .andExpect(jsonPath("$.[*].id").value(hasItem(uniqueCustomerID.getId().
	 * intValue()))); }
	 ...
	 * TestUtil.equalsVerifier(UniqueCustomerID.class); UniqueCustomerID
	 * uniqueCustomerID1 = new UniqueCustomerID(); uniqueCustomerID1.setId(1L);
	 ...
	 * TestUtil.equalsVerifier(UniqueCustomerIDDTO.class); UniqueCustomerIDDTO
	 * uniqueCustomerIDDTO1 = new UniqueCustomerIDDTO();
	 * uniqueCustomerIDDTO1.setId(1L); UniqueCustomerIDDTO uniqueCustomerIDDTO2 =
	 * new UniqueCustomerIDDTO();
